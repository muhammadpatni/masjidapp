...
class FirestoreService {
  ...
  // Path: users/{uid}/donations  — har masjid ka data alag
  CollectionReference get _donations =>
      _db.collection('users').doc(_uid).collection('donations');
  ...
  Future<void> addDonation(Donation d) => _donations.add(d.toFirestore());

  Future<void> updateDonation(Donation d) =>
      _donations.doc(d.id).update(d.toFirestore());

  Future<void> deleteDonation(String id) => _donations.doc(id).delete();
  ...
  Future<void> addExpense(Expense e) => _expenses.add(e.toFirestore());
  ...
  Future<void> addProject(Project p) => _projects.add(p.toFirestore());
